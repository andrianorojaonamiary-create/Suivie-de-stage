  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      stages.forEach((stage) => {
        if (stage.student?.id && !studentsById.has(stage.student.id)) {
          studentsById.set(stage.student.id, stage.student);
        }
      });
      const supervisedStudents = Array.from(studentsById.values());
      ...
      const mapped = supervisedStudents.map((info) => {
        const studentId = info.id;
        const stageList = stagesByStudent.get(studentId) || [];
        const stage = stageList.length > 0 ? prefer(stageList) : null;
        const m = stage ? mapInternship(stage) : null;
        ...
        return {
          id: studentId,
          nom: `${info.user?.prenom || ''} ${info.user?.nom || ''}`.trim() || 'Étudiant',
          matricule: info.matricule || '—',
          email: info.user?.email || '—',
          telephone: info.telephone || '—',
          filiere: info.formation || info.filiere || 'Non renseigné',
          niveau: info.niveau || info.level || 'Non renseigné',
          stage: m
            ? {
                id: m.id,
                titre: m.titre,
            ...
            : null,
          evaluation: stage?.statut === 'TERMINE' ? 'À faire' : '—',
          rapports: nbRapports,
        };
      });
                  ...
                  <tr key={student.id}>
                    ...
                    <td>
                      <div className="stage-cell">
                        <span className="stage-title">{student.stage?.titre || 'Aucun stage'}</span>
                    ...
                    <td>{getStatusBadge(student.stage?.statut || 'À venir')}</td>
