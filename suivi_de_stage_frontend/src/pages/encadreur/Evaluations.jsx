...
function EvalDetailModal({ evaluation, onClose }) {
  ...
  const getStatusClass = (statut) => {
    return statut === 'À faire' ? 'badge-en-attente' : 'badge-valide';
  };
...
function EncadreurEvaluations() {
  ...
  const stats = {
    ...
    valides: evaluations.filter(e => e.statut === 'Fait' || e.statut === 'Validé').length,
  ...
  const handleSaveEvaluation = async (data) => {
    try {
      ...
      await evaluationsApi.create({
        ...
        note: data.moyenne,
        appreciationGenerale: data.appreciation,
      });
      toast.success(<>
