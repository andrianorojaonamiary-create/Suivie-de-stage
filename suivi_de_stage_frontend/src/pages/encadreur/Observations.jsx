...
import { internshipsApi, studentsApi, trackingApi } from '../../api';
...
function EncadreurObservations() {
  ...
  const [selectedEtudiant, setSelectedEtudiant] = useState(
    studentId ? String(studentId) : 'tous'
  );
  ...
  useEffect(() => {
    const fetchObservations = async () => {
      try {
        setLoading(true);
        const [stagesResult, studentsResult] = await Promise.allSettled([
          internshipsApi.getAll({ limit: 100 }),
          studentsApi.getAll({ limit: 100 }),
        ]);
        if (stagesResult.status === 'rejected') throw stagesResult.reason;

        const res = stagesResult.value;
        const internships = res?.data || (Array.isArray(res) ? res : []);
        const studentList = studentsResult.status === 'fulfilled'
          ? studentsResult.value?.items ||
            studentsResult.value?.data ||
            (Array.isArray(studentsResult.value) ? studentsResult.value : [])
          : [];
  ...
  const studentLabel = (student) =>
    student && (student.user?.prenom || student.user?.nom)
      ? `${student.user?.prenom ?? ''} ${student.user?.nom ?? ''}`.trim()
      : 'Étudiant';
  ...
  const etudiantOptions = [
    ...
    ...observations
      .filter((observation) => observation.etudiantId)
      .map((observation) => ({
        value: String(observation.etudiantId),
        label: observation.etudiant || 'Étudiant',
      })),
  ]
    .filter((o, index, arr) => arr.findIndex((x) => x.value === o.value) === index);

  const currentEtudiantOption = (() => {
    if (!studentId) return null;
    const existing = etudiantOptions.find((o) => String(o.value) === String(studentId));
    if (existing) return null;
    const foundObs = observations.find((o) => String(o.etudiantId) === String(studentId));
    const foundStage = stages.find((s) => String(s.student?.id) === String(studentId));
    const label =
      foundObs?.etudiant ||
      studentLabel(foundStage?.student) ||
      'Étudiant';
    return { value: String(studentId), label };
  })();

  const finalEtudiantOptions = currentEtudiantOption
    ? [...etudiantOptions, currentEtudiantOption]
    : etudiantOptions;

  const studentFilterOptions = [
    { value: 'tous', label: 'Tous les étudiants' },
    ...finalEtudiantOptions,
  ];
  ...
  const handleAdd = async () => {
    if (!formData.etudiantId) {
      toast.warning('Veuillez sélectionner un étudiant');
      return;
    }
    if (!formData.contenu.trim()) {
      toast.warning('Veuillez saisir une observation');
      return;
    }
      ...
      <div className="page-header">
        <div>
          ...
          <p className="text-muted">
            {studentId ? `Observations de ${studentName}` : 'Gérez les observations sur les étudiants'}
          </p>
        </div>
        <button
          ...
          onClick={() => {
            setFormData({
              etudiantId: studentId ? String(studentId) : '',
              contenu: '',
            });
            ...
            {totalPages > 1 && (
              <div className="pagination">
                ...
                <span className="page-info">
                  {paginatedObs.length} observation{paginatedObs.length > 1 ? 's' : ''}
                </span>
              </div>
            )}
          </>
