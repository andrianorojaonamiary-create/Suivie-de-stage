    ...
    const fetchStudent = async () => {
      try {
        ...
        setStudent({
          ...
          filiere: data.formation || data.filiere || 'Non renseigné',
          niveau: data.niveau || data.level || 'Non renseigné',
          email: data.user?.email || '—',
          telephone: data.telephone || '—',
          promotion: data.promotion || 'Non renseignée',
          situationProfessionnelle: data.situationProfessionnelle || 'Non renseignée',
          statut: data.statutAcademique || 'ACTIF',
        });
      } catch (err) {
        try {
          const res = await internshipsApi.getAll({ limit: 100 });
          const stages = res?.data || (Array.isArray(res) ? res : []);
          const stage = stages.find(
            (item) => String(item.student?.id) === String(studentId),
          );
          const data = stage?.student;
          ...
          console.error('Erreur chargement étudiant:', fallbackError);
        }
      } finally {
        setLoading(false);
      }
      ...
      <div className="detail-content">
        {activeTab === 'info' && (
          <div className="info-fields-grid">
            ...
            <div className="info-field">
              <span className="info-field-label">Promotion</span>
              <span className="info-field-box">{student.promotion}</span>
            </div>
            <div className="info-field">
            ...
            <div className="info-field">
              <span className="info-field-label">Situation professionnelle</span>
              <span className="info-field-box">{student.situationProfessionnelle}</span>
            </div>
            <div className="info-field">
                  ...
                  {evaluations.map((evalItem) => (
                    <tr key={evalItem.id}>
                      ...
                      <td data-label="Statut">
                        <span className={`badge ${evalItem.statut === 'Validé' ? 'badge-valide' : 'badge-en-attente'}`}>
                          {evalItem.statut}
                        </span>
                      </td>
