...
import { 
  ...
  FaArrowRight, FaMapMarkerAlt, FaBell, FaTimes
} from 'react-icons/fa';
...
function EnseignantDashboard() {
  ...
  const readNotification = async (activity) => {
    if (activity.read || !activity.id) return;
    try {
      await notificationsApi.markAsRead(activity.id);
    } finally {
      setRecentActivities(prev => prev.map(item => item.id === activity.id ? { ...item, read: true } : item));
    }
  };
        ...
        <div className="enseignant-activities">
          ...
          <div className="activities-list">
            {recentActivities.map((activity) => (
              <div key={activity.id} className={`activity-item ${!activity.read ? 'unread' : ''}`} onClick={() => readNotification(activity)}>
                ...
                {activity.read && activity.id && (
                  <button type="button" className="activity-delete" aria-label="Supprimer la notification" title="Supprimer la notification" onClick={(event) => { event.stopPropagation(); deleteNotification(activity.id); }}>
                    <FaTimes />
                  </button>
                )}
              </div>
